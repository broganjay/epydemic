...
class CompartmentedModel(Process):
    ...
    T_OCCUPIED: str = (
        "tOccupied"  #: State variable holding the occupation time of an edge.
    )
    T_HITTING: str = "tHitting"  #: State variable holding the infection time of a node.
    HITTING_PROCESS_NAME: str = (
        "hittingProcess"  #: State variable holding the name of the infecing process.
    )
    ...
    def __init__(self, name: Optional[str] = None):
        ...
        self._compartments: Dict[str, float] = (
            dict()
        )  # compartment -> initial probability
    ...
    def initialCompartmentDistribution(self) -> List[Tuple[str, float]]:
        ...
        if not math.isclose(a, 1.0):
            raise ValueError(
                "Bad initial compartment distribution (probabilities of model don't sum to one)"
            )
